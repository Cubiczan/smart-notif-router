...
### Rows 18 / 19 — degradation ladder and protocol health probes: ADOPTED

**Row 18 (degradation ladder, organic adoption):** the classification pipeline degrades in explicit, evidence-carrying tiers (`src/services/ai-router.service.ts`): AI routing (GLM-4-Plus via z-ai-web-dev-sdk, bounded by an AbortController + hard `withTimeout` race) falls back to `getDefaultRouting` — a deterministic keyword-heuristic router — on **every** failure path: AI disabled by config, client-init failure, completion timeout (the race rejects → catch → default), invalid JSON (`parseAIResponse` returns null → default), or any thrown error (catch → default). The fallback is evidence-carrying: it stamps `confidence: 0.3` and `reasoning: 'Fallback heuristic routing (AI unavailable)'`, so consumers can always distinguish AI-routed from degraded routing. Delivery-side degradation uses the status enum (`pending|routed|delivered|failed`) with an idempotent retry endpoint that appends delivery logs rather than overwriting them. Verified by `POST /api/notifications returns 201 and exercises the AI fallback`.

**Row 19 (protocol health probes):** the pre-adoption route was the row-19 anti-pattern — `status: 'healthy'` returned unconditionally (even with the database down), Redis was reported as a connection **URL string** implying connectivity that was never checked, and nothing but the DB was probed. The route now (`src/routes/health.ts`): derives top-level `status` from the database probe (`healthy` | `degraded` — the DB is the only hard dependency for serving requests); probes Redis with a bounded (750 ms) TCP handshake — connectivity only, no AUTH sent, failures surfaced as namespaced reason codes (`REDIS_ECONNREFUSED`, `REDIS_HANDSHAKE_TIMEOUT`, `REDIS_UNREACHABLE`), never raw error text; reports AI as **configuration state only** (`enabled`, `engine`, `probed: false`) — deliberately not probed with a live model call, because health checks must not spend model tokens or make liveness depend on model availability; and documents the liveness contract (200-always, dependency failures encoded in the body) in the OpenAPI description. Tests: reason codes on unreachable Redis, no internal URLs or raw error strings in the envelope, AI config-only, 200-always under dependency failure.
